...
describe('getCampaignSendPreflight', () => {
  it('builds a blocked preflight summary from sender plan, draft approval, and recipient resolution', async () => {
    ...
    const campaignIn = vi.fn().mockResolvedValue({
      data: [{ id: 'camp-source', name: 'Source Campaign' }],
      error: null,
    });
    const campaignSelect = vi.fn().mockReturnValue({ eq: campaignEq, in: campaignIn });
    ...
    const draftsEq = vi.fn().mockResolvedValue({
      data: [
        { id: 'draft-1', status: 'approved', contact_id: 'contact-1', email_type: 'intro' },
        { id: 'draft-2', status: 'approved', contact_id: 'contact-2', email_type: 'intro' },
        { id: 'draft-3', status: 'generated', contact_id: 'contact-3', email_type: 'intro' },
        { id: 'draft-4', status: 'rejected', contact_id: 'contact-4', email_type: 'intro' },
      ],
      error: null,
    });
    ...
    const outboundsSelect = vi.fn().mockReturnValue({ in: outboundsIn });
    ...
    expect(result.issues.find((row) => row.code === 'missing_recipient_email')).toMatchObject({
      draftId: 'draft-2',
      contactId: 'contact-2',
    });
  ...
  it('marks campaign paused and no-sendable-drafts blockers when applicable', async () => {
    ...
    const draftsEq = vi.fn().mockResolvedValue({
      data: [
        { id: 'draft-10', status: 'approved', contact_id: 'contact-10', email_type: 'intro' },
        { id: 'draft-11', status: 'rejected', contact_id: 'contact-11', email_type: 'intro' },
      ],
      error: null,
    });
    const draftsSelect = vi.fn().mockReturnValue({ eq: draftsEq });
    ...
    expect(result.summary.sendableApprovedIntroDraftCount).toBe(0);
    expect(result.summary.sendableApprovedBumpDraftCount).toBe(0);
    expect(result.summary.approvedSuppressedContactCount).toBe(0);
    expect(result.issues.map((row) => row.code)).toEqual(['missing_recipient_email']);
  ...
  it('blocks approved drafts for suppressed contacts even when recipient email is present', async () => {
    ...
    const draftsIn = vi.fn().mockResolvedValue({
      data: [{ id: 'draft-older', email_type: 'intro' }],
      error: null,
    });
    ...
    expect(result.issues.map((row) => row.code)).toEqual(['suppressed_contact']);
  });

  it('blocks approved intro drafts when the contact already received an intro in another campaign', async () => {
    const campaignSingle = vi.fn().mockResolvedValue({
      data: {
        id: 'camp-4',
        name: 'Repeat Intro Campaign',
        status: 'ready',
        segment_id: 'seg-4',
        segment_version: 1,
        created_at: '2026-03-20T10:00:00Z',
    ...
    const draftsEq = vi.fn().mockResolvedValue({
      data: [
        { id: 'draft-40', status: 'approved', contact_id: 'contact-40', email_type: 'intro' },
      ],
      error: null,
    });
    const draftsIn = vi.fn().mockResolvedValue({
      data: [{ id: 'draft-other-campaign', email_type: 'intro' }],
      error: null,
    });
    const draftsSelect = vi.fn().mockReturnValue({ eq: draftsEq, in: draftsIn });
      ...
      data: [
        {
          ...
          draft_id: 'draft-other-campaign',
          status: 'sent',
          sent_at: '2026-03-31T11:18:28.055+00:00',
        },
      ],
      error: null,
    });
    ...
    const result = await getCampaignSendPreflight(supabase, 'camp-4');
    ...
    expect(result.summary.sendableApprovedBumpDraftCount).toBe(0);
    expect(result.summary.approvedSuppressedContactCount).toBe(1);
    expect(result.issues.map((row) => row.code)).toEqual(['intro_already_sent']);
    expect(result.issues[0]).toMatchObject({
      relatedOutboundId: 'out-40',
      relatedDraftId: 'draft-other-campaign',
      relatedCampaignId: 'camp-source',
      relatedCampaignName: 'Source Campaign',
      relatedSentAt: '2026-03-31T11:18:28.055+00:00',
    });
    expect(draftsIn).toHaveBeenCalledWith('id', ['draft-other-campaign']);
  });
});
