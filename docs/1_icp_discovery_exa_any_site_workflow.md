...
- Core app integration:
  - Exa is integrated via a small HTTP client for discovery/search (Websets and related APIs).
  - AnySite is integrated via a small HTTP client used **only for enrichment** (LinkedIn/social/web parsing) once companies/contacts exist in Supabase.
- Optional MCP façade:
  - `exa-mcp-server` and `anysite-mcp` may be added as thin façades for external agents (Claude Desktop, code agents) but sit on top of the same HTTP-based interfaces.
  - Agents see only the minimal tools they need (principle of least power); the CLI and Web adapter do not depend on the full 60+ tool surface.
