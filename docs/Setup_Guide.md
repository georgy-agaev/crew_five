...
2. Populate:
   ...
   - Research/outreach:  
     - `EXA_API_KEY` (Exa search/Websets; optional `EXA_API_BASE`)  
     - `PARALLEL_API_KEY` (Parallel.ai; optional `PARALLEL_API_BASE`, defaults to `https://api.parallel.ai`)  
     - `FIRECRAWL_API_KEY` (Firecrawl.dev; optional `FIRECRAWL_API_BASE`, defaults to `https://api.firecrawl.dev`)  
     - `SMARTLEAD_API_KEY`, `LEADMAGIC_API_KEY`
...
## 10. Database Migrations & View Refresh
- Apply Supabase migrations before running CLI/Web:
  ```bash
  supabase db reset   # destructive; use only on local dev DB
  # or, to apply latest migrations on a dev project
  supabase db push
  ```
- Migrations to note for W0.v3/W1.v2/AN.v2 readiness:
  - `20251201120000_add_email_event_fk_columns.sql` (email_events analytics FKs)
  - `20251201120500_update_analytics_events_flat_view.sql` (view prefers event-level FKs)
- After applying, validate locally:
  ```bash
  pnpm vitest run tests/emailEvents.test.ts tests/analytics.test.ts
  ```
