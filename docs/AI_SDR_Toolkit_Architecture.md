...
- ICP Persona Research Coach prompt lives at `prompts/ICP_Persona_Reseach_Coach_v1_0.md` and is loaded by `src/services/icpCoach.ts` / `src/services/coach.ts` in an “express JSON mode” for `/api/coach/icp` and related flows (Web UI Workflow 1 and CLI `icp:coach:*` commands).
...
- **Research adapters:** Exa Websets (discovery/search) and enrichment providers (AnySite, Firecrawl, Parallel) share a unified interface behind small HTTP clients (e.g., “discover candidates”, “fetch company insights”), with caching and retry policies; discovery and enrichment remain separate concerns.
