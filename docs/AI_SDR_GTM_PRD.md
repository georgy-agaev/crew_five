...
**Implementation status (v0.4, 2025-12-01)**
- W0.v3 – Enrichment entry point on finalized segments is implemented (segment snapshots, enrichment jobs, and research fields).
- W1.v2 – ICP profiles/hypotheses and split coach prompts are implemented and wired into draft generation.
- AN.v2 – Baseline analytics and prompt/pattern optimization helpers are implemented, with CLI support.
- W2 (SIM) – Contracts and `sim` jobs are defined (Option 2), but SIM modes (W2.v0–v2) are not yet executed; all SIM requests resolve to `not_implemented` in the current build.
